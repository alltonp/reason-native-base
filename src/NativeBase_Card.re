...
let convertSectionsRowSelections = (sections: array(sectionRowSelections)) => {
  Js.Dict.fromArray(
    Belt.Array.map(sections, ((sectionId, rows)) =>
      (sectionId, Js.Dict.fromArray(rows))
    ),
  );
};
...
external make:
  (
    ...
    ~renderRow: ('rowData, sectionId, rowId, rowSelected) => React.element=?,
    ~renderScrollComponent: 'props => ReactNative.ScrollView.element=?,
    ~renderSectionHeader: ('rowData, sectionId) => React.element=?,
    ~renderSeparator: (sectionId, rowId, rowSelected) => React.element=?,
...
let make =
  ...
  make(
    ...
    ~onChangeVisibleRows=
      (a, b) =>
        onChangeVisibleRows(
          convertSectionsRowSelections(a),
          convertSectionsRowSelections(b),
        ),
    ~onEndReached,
    ~onEndReachedThreshold,
    ~pageSize,
